...
export default function MainPage() {
  ...
  useEffect(() => {
    if (!isSignedIn) {
      window.location.href = "/"; // 로그인 안 했으면 로그인 페이지로 이동
    }
  }, [isSignedIn]);
  ...
  if (!isSignedIn) return null; // 로그인 확인 전에는 렌더링 X

  return (
    <div className="flex min-h-screen bg-slate-50">
      {/* Sidebar */}
      <aside className="flex flex-col w-64 bg-white shadow-md py-10">
        <div className="flex flex-col items-center px-4 pb-4 border-b border-slate-200">
          ...
          />
          <h2 className="text-base font-semibold text-slate-800">홍길동</h2>
          <p className="text-sm text-slate-500">hong@example.com</p>
          
          <SignOutButton>
            <button className="flex items-center gap-1 text-slate-400 mt-5">
        ...
        <nav className="flex-1 mt-6 space-y-2">
          <a className="flex items-center gap-3 px-7 py-4 text-blue-600 rounded-lg hover:bg-slate-100">
            <span className="material-symbols-outlined text-2xl">checklist</span>
            <span className="font-medium">Tasks</span>
          </a>
          <a className="flex items-center gap-3 px-7 py-4 text-slate-600 rounded-lg hover:bg-slate-100">
            ...
            {tasks.map((task, index) => (
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    e.target.checked
                      ? e.currentTarget.nextElementSibling?.classList.add("line-through", "text-slate-400")
                      : e.currentTarget.nextElementSibling?.classList.remove("line-through", "text-slate-400")
                  }
                />
